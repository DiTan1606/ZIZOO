...
const getHotelCategory = (priceLevel, rating) => {
  ...
  if (priceLevel >= 3) return 'luxury'; // Fallback dựa trên rating
...
export const createRealTimeItinerary = async (prefs, userId, mapInstance) => {
  ...
  console.log('Bắt đầu tạo itinerary với prefs:', prefs); // === THÊM MỚI: TÍNH TOÁN NGÂN SÁCH & PHÂN LOẠI ===
  ...
  console.log(
    `Budget per person/day: ${budgetPerPersonPerDay.toFixed(
      0
  ...
  try {
    ...
    if (riskScore > 0.7) {
      risky.push(province);
      alerts.push(
        `${province}: Rủi ro cao (bão/lũ) - Điểm rủi ro: ${(
          riskScore * 100
        ).toFixed(1)}%`
      );
    }
  ...
  if (risky.length > 0) {
    throw new Error(
      `Khu vực ${province} có rủi ro thiên tai cao. Vui lòng chọn địa điểm khác.`
    );
  } // === KHỞI TẠO PLACES SERVICE ===

  await initPlacesService(mapInstance); // === TÌM ĐIỂM ĐẾN ===
  ...
  const placeTypes = types.flatMap(
    (t) => typeToPlaces[t] || ['tourist_attraction']
  );
  if (ecoFriendly) placeTypes.push('park');
  if (adventureLevel > 3) placeTypes.push('hiking_area'); // Lấy điểm đến cho mỗi loại
  ...
  for (const type of [...new Set(placeTypes)].slice(0, 4)) {
    try {
      ...
      console.log(
        `Tìm thấy ${results.length} kết quả cho ${type} ở ${province}`
      ); // SỬA: Lọc theo ngân sách (price_level) TRƯỚC KHI SLICE

      const spots = results
        .filter((p) => {
          const ratingOk = p.rating >= 3.8; // price_level cho điểm tham quan thường không chính xác (hoặc 0) // Tạm thời nới lỏng cho budget low: ưu tiên 0, 1, 2
        ...
        .map((p) => {
          ...
          const dailyPlan = splitIntoDays(optimizedRoute, days); // SỬA: Sử dụng giá vé thực tế thay vì ước lượng từ price_level
          const ticketPrice = getRealTicketPrice(type, normalizedProvince);
          const estimatedPrice =
            p.price_level !== undefined
              ? estimatePricePerPerson(p.price_level, 'attraction')
              : ticketPrice;
  ...
  const optimizedRoute = optimizeRoute(allDestinations);
  const dailyPlan = splitIntoDays(optimizedRoute, days); // === GỢI Ý ĂN UỐNG ===

  const meals = []; // THÊM: Lấy mức giá nhà hàng phù hợp
  ...
  for (const day of dailyPlan) {
    ...
    try {
      ...
      const filteredLunch = lunchResults
        .filter((r) =>
          allowedRestaurantPriceLevels.includes(
            r.price_level !== undefined ? r.price_level : 2
          )
        ) // Mặc định là 2 (Medium)
        .sort(
          (a, b) => (b.user_ratings_total || 0) - (a.user_ratings_total || 0)
        );
    ...
    try {
      ...
      const dinnerKeyword =
        budgetCategory === 'high'
          ? 'nhà hàng 5 sao, đặc sản cao cấp'
          : 'nhà hàng, đặc sản';
    ...
    const fallbackLunchPrice =
      budgetCategory === 'low'
        ? 60000
        : budgetCategory === 'high'
        ? 150000
        : 80000;
    const fallbackDinnerPrice =
      budgetCategory === 'low'
        ? 80000
        : budgetCategory === 'high'
        ? 300000
    ...
    meals.push({
      lunch: lunch
        ? {
            ...
            photo: lunch.photos?.[0]
              ? getPhotoUrl(lunch.photos[0].photo_reference)
              : null,
            price:
              estimatePricePerPerson(lunch.price_level, 'restaurant') *
              travelers,
          }
        : {
      ...
      dinner: dinner
        ? {
            ...
            photo: dinner.photos?.[0]
              ? getPhotoUrl(dinner.photos[0].photo_reference)
              : null,
            price:
              estimatePricePerPerson(dinner.price_level, 'restaurant') *
              travelers,
          }
        : {
  ...
  try {
    ...
    const categorizedHotels = hotelResults
      ...
      .slice(0, 10); // Lấy nhiều hơn để phân loại (tăng từ 5 lên 10) // PHÂN LOẠI: Budget, Mid-range, Luxury // SỬA: Dùng hàm getHotelCategory để phân loại

    const budgetHotels = categorizedHotels.filter(
      (h) => getHotelCategory(h.price_level, h.rating) === 'budget'
    );
    const midRangeHotels = categorizedHotels.filter(
      (h) => getHotelCategory(h.price_level, h.rating) === 'mid-range'
    );
    const luxuryHotels = categorizedHotels.filter(
      (h) => getHotelCategory(h.price_level, h.rating) === 'luxury'
    ...
    if (selectedHotels.length === 0 && categorizedHotels.length > 0) {
      ...
      const remainingCategorized = categorizedHotels.filter(
        (h) => !allSelectedIds.has(h.place_id)
      );
      selectedHotels.push(
        ...remainingCategorized.slice(0, 3 - selectedHotels.length)
      );
    } // Chỉ lấy tối đa 3 khách sạn gợi ý
    const finalHotelSuggestions = selectedHotels.slice(0, 3);

    hotels.push(
      ...finalHotelSuggestions.map((h) => {
        ...
        const basePrice = estimatePricePerPerson(h.price_level, 'hotel'); // Giá phòng có thể điều chỉnh theo rating và số người
        let pricePerNight = basePrice;
        if (h.rating >= 4.5) pricePerNight *= 1.3;
        else if (h.rating >= 4.0) pricePerNight *= 1.1; // Nếu số người lớn hơn 2, có thể tính thêm phụ phí
        ...
        return {
          ...
          photo: h.photos?.[0]
            ? getPhotoUrl(h.photos[0].photo_reference)
            : null,
    ...
    console.warn('Lỗi tìm khách sạn:', err); // SỬA: FALLBACK KHÁCH SẠN MẶC ĐỊNH THEO NGÂN SÁCH
    let fallbackHotel;
    if (budgetCategory === 'high') {
      fallbackHotel = {
  ...
  try {
    ...
    const festival = festivalData.find(
      (f) => f.Province === normalizedProvince
    );
    festivalInfo = festival ? `Có ${festival.FestivalCount} lễ hội` : null;
  } catch (err) {
    console.warn('Lỗi lấy lễ hội:', err);
  } // === TÍNH TOÁN CHI PHÍ === // SỬA: Lấy fallback cost theo budget

  let fallbackHotelCost = 600000;
  if (budgetCategory === 'high') fallbackHotelCost = 2000000;
  else if (budgetCategory === 'low') fallbackHotelCost = 400000; // Sửa: hotels[0] là khách sạn ưu tiên theo budget
  const hotelCost = (hotels[0]?.pricePerNight || fallbackHotelCost) * days;
  const foodCost = meals.reduce(
    (sum, meal) => sum + meal.lunch.price + meal.dinner.price,
    0
  );
  const entranceCost =
    allDestinations.reduce((sum, dest) => sum + dest.pricePerPerson, 0) *
    travelers; // SỬA: Giá vé * số người
  const transportCost = 200000 * days;

  const totalCost = hotelCost + foodCost + entranceCost + transportCost;
  const remainingBudget = budget - totalCost; // === ĐỊNH DẠNG KẾT QUẢ ===
