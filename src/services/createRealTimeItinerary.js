...
const fetchRealDestinations = async (province, types, ecoFriendly, adventureLevel, mapInstance, budget, safeProvinces) => {   
    ...
    let destinations = [];
    let minRating = 4.0; // Bắt đầu với rating 4.0
    const maxAttempts = 2; // Thử lại với rating thấp hơn nếu cần
        ...
        for (const type of placeTypes.slice(0, 3)) {
            try {
                ...
                const top = results.slice(0, 2).map(p => ({
                    name: p.name,
                    address: p.vicinity,
                    lat: p.lat, // Đã đảm bảo lat/lng là số trong searchNearbyPlaces
                    lng: p.lng,
                    rating: p.rating,
                ...
                console.log(`Tìm thấy ${top.length} địa điểm loại ${type} ở ${province} với minRating=${minRating}`);
            } catch (err) {
                console.warn(`Lỗi tìm kiếm ${type} ở ${province} với minRating=${minRating}:`, err);
            }
        }
        minRating -= 0.5; // Giảm yêu cầu rating nếu không tìm thấy
    }
...
for (const p of provinces) {
    try {
        ...
        safeProvinces.push(p); // Chấp nhận tỉnh nếu không lấy được dữ liệu rủi ro
    }
}
...
    }
    if (allDestinations.length === 0) throw new Error('Không tìm thấy điểm đến phù hợp. Vui lòng thử lại với các tùy chọn khác.');

    // SỬA LỖI: Đảm bảo chuyển đổi tọa độ một cách an toàn
    allDestinations = allDestinations.map(d => ({ 
        ...d, 
        lat: Number(d.lat) || d.lat, 
        lng: Number(d.lng) || d.lng,
        // DÒNG NÀY ĐÃ ĐƯỢC THÊM TRONG CÁC PHIÊN BẢN TRƯỚC: allDestinations.map(d => ({ ...d, lat: Number(d.lat), lng: Number(d.lng) }));
    })); 
